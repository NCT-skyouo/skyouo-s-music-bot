...
const Genius = require(path.join(__dirname, 'libs', 'genius', 'index.js'))
...
var API = new (require('./libs/pluginAPI/API'))(bot, player)

for (const name in pluginsFiles) {
  try {
    const plobj = require('./plugins/' + pluginsFiles[name])
    var plugin = new plobj.Plugin(bot, API);
    if (!plobj.enable) continue
    plugin.onLoad()
    plugin.onEnable()
    processLogger.ok("載入插件檔案 '" + pluginsFiles[name] + "@" + plobj.version + "' 成功! (作者: " + plobj.author +")")
  } catch (e) {
    processLogger.error(e.toString())
    processLogger.trace(e.stack)
  }
