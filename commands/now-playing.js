  ...
  run: async (bot, msg, args) => {
    ...
    try {
      ...
      collector.on("collect", async (r) => {
        ...
        if (!bot.player.getQueue(msg.guild.id)) return
        switch (r.emoji.name) {
          case "❌":
            collector.stop()
            break
          ...
          case "🔀":
            ...
            break
          case "🔁":
            bot.commands.get("queuerepeat").run(bot, msg, args)
            break
          case "🔂":
          ...
          case "📤":
            ...
            try {
              var res = await mes.channel.awaitMessages(m => msg.author.id === m.author.id, { max: 1, time: 60000, errors: ['time']}).catch(e => { throw e })
              bot.commands.get("export").run(bot, res.first(), res.first().content.split(" "))
            } catch (e) {}
            break
        }
      })
