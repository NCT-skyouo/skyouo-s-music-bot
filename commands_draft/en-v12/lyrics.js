...
module.exports = {
    name: 'lyrics',
    category: 'music',
    description: 'Get the lyrics from mojim.com/genius.com',
    aliases: ['l'],
    run: async (bot, msg, args) => {
        const { player, MessageEmbed, config, mojim, genius } = bot
        ...
        try {
            ...
            if (!suffix) {
                if (!queue.playing) return message.edit(new MessageEmbed().setAuthor('No song provided!', msg.guild.iconURL()).setColor('FFEE00').setFooter(config.footer, bot.user.displayAvatarURL()))
            ...
            const menu = new MessageMenu()
                .setID('lyrics-response')
                .setPlaceholder('Choose the the search result that you think is the closest!')
                .setMaxValues(1)
                .setMinValues(1)
            ...
            message.edit(embed, menu).then(m => {
                ...
                collector.on('collect', async (menu) => {
                    ...
                    if (resByClient[0] === 'no') {
                        ...
                        if (!res) {
                            collector.stop()
                            return msg.channel.send(new MessageEmbed().setAuthor('❌ No lyrics found!!', msg.guild.iconURL()).setColor('FF0007').setFooter(config.footer, bot.user.displayAvatarURL()), {})
                        ...
                        const lyricembed = new MessageEmbed()
                            ...
                            .setDescription('**Source: Mojim Lyrics**\n' + res.split(/\n{2,4}/g).join('\n\n'))
                            .setColor('00FE37')
                            .setFooter(config.footer, bot.user.displayAvatarURL())
